"""
猜中国古人（AI海龟汤 v2.0）—— 模块化 Prompt 重构版（全古代 200 人版）

以 app_02_01.py 为框架，知识库切换为《top200_famous_people.csv》（中国全古代 200 名人），
是 app_02_ming.py 的姊妹版本（明朝 200 人 → 全古代 200 人）。

重构要点（对比 app_01_02.py）：
...
架构：
    ...
    top200_famous_people.csv  —— 知识库（中国全古代 200 名人）
    app_02_mini.py            —— 本文件，Streamlit 应用层
...
# =========================
# 📄 读取人物库（中国全古代 200 名人 CSV）
# =========================
def load_people_names():
    """依次尝试多种编码读取 CSV 文件，兼容 UTF-8、GBK、GB18030 等编码。

    知识库：《top200_famous_people.csv》
    读取列：人名
    """
    csv_path = "top200_famous_people.csv"
    encodings_to_try = ["utf-8", "utf-8-sig", "gbk", "gb2312", "gb18030"]
    for enc in encodings_to_try:
        try:
            with open(csv_path, "r", encoding=enc) as f:
                reader = csv.DictReader(f)
                names = [row["人名"].strip() for row in reader if row.get("人名", "").strip()]
                if names:
                    return names
...
st.title("🎭 猜中国古人（AI海龟汤 v2.0 模块化版）")
st.caption("🔧 架构升级：Prompt 分层 + Python 状态逻辑（N>=6 判定已迁移至代码层）｜ 知识库：中国全古代 200 名人")
...
# 用户输入
user_input = st.chat_input('输入"开始"或"再来一局"启动游戏（中国古人）')
