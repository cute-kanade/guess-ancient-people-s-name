"""
猜明朝人物（AI海龟汤 v3.0）—— 全量重构明朝主题版

以 app_03_01.py 为框架，知识库切换为《明朝名人二百人表》CSV，
迭代掉 app_02_ming.py。

对比 app_02_ming.py 的改进：
    ...
    ✅ 人物库升级：CSV 人名 + JSON 结构化字段合并，默认朝代补"明"
...
# =========================
MING_THEME = "明朝历史"
MING_CSV_PATH = "明朝名人二百人表 - 明朝名人二百人表.csv"
MING_DEFAULT_DYNASTY = "明"
...
# =========================
@st.cache_data
def load_ming_people_cached():
    """加载明朝人物库：CSV 人名 + JSON 结构化字段合并，默认朝代补"明"。"""
    return load_people_from_csv(
        csv_path=MING_CSV_PATH,
        default_dynasty=MING_DEFAULT_DYNASTY,
    )
...
# 🎮 Streamlit UI
# =========================
st.title("🎭 猜明朝人物（AI海龟汤 v3.0 全量重构版）")
st.caption("🔧 架构分层 + 结构化人物库 + 批量重答核查 | 知识库：明朝名人二百人表 | 旧版见 legacy/")
...
# ⌨️ 用户输入
# =========================
user_input = st.chat_input('输入"开始"或"再来一局"启动游戏（明朝人物）；怀疑答案可输入"重答""我怀疑"等')
...
if user_input:
    ...
    if is_start:
        persons = load_ming_people_cached()
        if not persons:
            st.error("明朝人物库加载失败，请确认 CSV 文件存在。")
